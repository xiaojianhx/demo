...
public class Router2Handler implements Handler<RoutingContext> {
    ...
    public void handle(RoutingContext rc) {
        ...
        mySQLClient.rxGetConnection().subscribe(conn -> {
            ...
            Single<ResultSet> resa = conn.rxQuery("select * from goods where id = 14910").flatMap(
                    r -> conn.rxQueryWithParams("select * from stores where id = ?", new JsonArray().add(r.getRows().get(0).getLong("store_id"))))
